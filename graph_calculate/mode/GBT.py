...
def train_and_evaluate_model(training_data):
    ...
    # 将大于等于1的值视为正类（1），小于1的值视为负类（0）
    training_data = training_data.withColumn("indexed_edge_type", expr("case when edge_type >= 1 then 1 else 0 end"))

    gbt = GBTClassifier(featuresCol="features", labelCol="indexed_edge_type", maxIter=10, maxBins=300)
